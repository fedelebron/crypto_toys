...
int main(void) {
  char plaintext[114] = "Ladies and Gentlemen of the class of '99: If I could "
                        ...
                        "would be it.";
  size_t plaintext_len = 114;
  
  char additional_data[12] = {'P', 'Q', 'R', 'S', 0xc0, 0xc1,
                              0xc2, 0xc3, 0xc4, 0xc5, 0xc6, 0xc7};
  size_t additional_data_len = 12;
  ...
  uint8_t tag[16];
  aead_chacha20_poly1305_encrypt(key, nonce, plaintext, plaintext_len, 
                                 additional_data, additional_data_len, tag);
  
  printf("Ciphertext:\n");
  for (int i = 0; i < 8; ++i) {
    for (int j = 0; j < 16; ++j) {
      int idx = 16 * i + j;
      if (idx >= plaintext_len) break;
  ...
  int success = aead_chacha20_poly1305_decrypt(
      key, nonce, plaintext, plaintext_len, 
      additional_data, additional_data_len, tag);
